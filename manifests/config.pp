...
class ferm::config {
  ...
  -> file{'/etc/ferm.d/definitions/10-netadmin.conf':
    ensure => 'file',
    source => "puppet:///modules/${module_name}/10-netadmin.conf",
  }
  -> file{'/etc/ferm.d/definitions/20-manual.conf':
    ensure => 'file',
    source => "puppet:///modules/${module_name}/20-manual.conf",
  }
